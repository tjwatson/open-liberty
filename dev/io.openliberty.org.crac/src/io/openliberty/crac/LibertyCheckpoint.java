...
public class LibertyCheckpoint {
    ...
    static class CRaCCheckpointHook implements CheckpointHook {
        private final Deque<CheckpointHook> hooks = new ConcurrentLinkedDeque<CheckpointHook>();

        void registerResource(Resource r, Context<? extends Resource> c) {
    ...
    static final CRaCCheckpointHook hooks;
    static {
        CRaCCheckpointHook newHooks = new CRaCCheckpointHook();
        CheckpointPhase phase = CheckpointPhase.getPhase();
        if (phase != CheckpointPhase.INACTIVE) {
            debug(tc, () -> "adding CheckpointHook for CRaC resources.");
            phase.addMultiThreadedHook(newHooks);
        }
        hooks = newHooks;
    }

    static public void register(Resource r, Context<? extends Resource> c) {
        hooks.registerResource(r, c);
    }
